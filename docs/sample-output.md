...
> **Important:** This is fictional, illustrative sample output for an early prototype. It is not a validated scientific review. It does not cite real papers and should not be treated as biomedical evidence. BioSignals is a research-navigation and mechanism-mapping prototype, not a clinical tool.

## Demo domain

ALS / neurodegeneration, with emphasis on motor neuron vulnerability and overlapping stress mechanisms.
...
This pattern does **not** imply a single cause of ALS, a validated disease model, or a clinical recommendation. It suggests a candidate failure architecture where several mechanisms may reinforce each other under specific cellular contexts.
...
| `motor_neuron_vulnerability` | Motor neuron vulnerability | phenotype | Reduced resilience of motor neurons under stress. |

## Example edges

| Source | Relationship type | Target | Evidence strength | Uncertainty | Convergence tags |
| --- | --- | --- | --- | --- | --- |
| `mitochondrial_dysfunction` | `contributes_to` | `oxidative_stress` | moderate | Context may differ by model and disease stage. | energy-stress, oxidative-burden |
| `calcium_dysregulation` | `context_dependent` | `mitochondrial_dysfunction` | moderate | Directionality may be bidirectional. | calcium-mitochondria-loop |
| `excitotoxicity` | `contributes_to` | `calcium_dysregulation` | low_to_moderate | Depends on receptor subtype, cell state, and assay system. | excitability-stress |
| `protein_aggregation` | `associated_with` | `rna_processing_defects` | low | Relationship may be indirect or mediated by specific proteins. | proteostasis-rna |
| `neuroinflammation` | `context_dependent` | `motor_neuron_vulnerability` | mixed | Glial effects may be protective or harmful depending on context. | glial-context |
...
```json
{
  ...
  "context": "Fictional ALS/neurodegeneration sample set used only to demonstrate output shape.",
  "caveats": [
...
## Example unresolved research questions

- Which ALS model systems support mitochondrial dysfunction upstream of calcium dysregulation?
- Which findings are specific to motor neurons versus shared across neural and glial cell types?
- Are inflammatory signals causal contributors, compensatory responses, or both depending on disease stage?
- What evidence would distinguish a convergence pattern from a loose cluster of correlated stress markers?

## Example plain-language research brief
